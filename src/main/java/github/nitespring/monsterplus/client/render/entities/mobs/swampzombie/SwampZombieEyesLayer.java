...
public class SwampZombieEyesLayer<T extends SwampZombie> extends RenderLayer<T, SwampZombieModel<T>> {
	private static final ResourceLocation TEXTURE_LOCATION = ResourceLocation.fromNamespaceAndPath(MonsterPlus.MODID, "textures/entities/swamp_zombie_eyes.png");

   private final DrownedModel<T> model;

   public SwampZombieEyesLayer(RenderLayerParent<T, SwampZombieModel<T>> p_174490_, EntityModelSet p_174491_) {
      super(p_174490_);
      this.model = new DrownedModel<>(p_174491_.bakeLayer(ModelLayers.PLAYER));
   }
