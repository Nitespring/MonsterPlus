...
public class CurseflameFireball extends AbstractHurtingProjectile implements ItemSupplier{
    ...
    public CurseflameFireball(EntityType<? extends AbstractHurtingProjectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public CurseflameFireball(EntityType<? extends AbstractHurtingProjectile> pEntityType, double pX, double pY, double pZ, Level pLevel) {
        super(pEntityType, pX, pY, pZ, pLevel);
    }

    public CurseflameFireball(EntityType<? extends AbstractHurtingProjectile> pEntityType, double pX, double pY, double pZ, Vec3 pMovement, Level pLevel) {
        super(pEntityType, pX, pY, pZ, pMovement, pLevel);
    }

    public CurseflameFireball(EntityType<? extends AbstractHurtingProjectile> pEntityType, LivingEntity pOwner, Vec3 pMovement, Level pLevel) {
        super(pEntityType, pOwner, pMovement, pLevel);
    }

    @Override
    public ItemStack getItem() {
        return ItemInit.CURSEFLAME_POWDER.get().getDefaultInstance();
    }
    ...
    public void tick() {
        ...
        if(!this.isInWater()) {
            for (int i = 0; i < 2; i++) {
                this.level().addAlwaysVisibleParticle(ParticleInit.CURSEFLAME.get(),
